...
<script type="text/javascript">
	...
	jQuery(function($) {
		//取用户的样式
		var uis=ajaxByurl("rest/menu/getUIstyle");
		if(uis!=null){
			//改变皮肤
			$("#skin-colorpicker").get(0).selectedIndex = uis.skin;
			$("#skin-colorpicker").change();
			//界面调整
			$("#ace-settings-add-container").get(0).checked =boolConvert(uis.uiStyle);
			$("#ace-settings-add-container").click();
			//子菜单悬浮
			$("#ace-settings-hover").get(0).checked = boolConvert(uis.submenu);
			$("#ace-settings-hover").click();
			//迷你导航
			$("#ace-settings-compact").get(0).checked = boolConvert(uis.miniNav);
			$("#ace-settings-compact").click();
			//导航边框切换
			$("#ace-settings-highlight").get(0).checked = boolConvert(uis.navStyle);
